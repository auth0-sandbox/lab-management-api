...
app.get("/profile", async (req, res) => {
    ...
    try {
        if (locals.user) {
            const response = await auth0ManagementClient.users.get({ id: req.oidc.idTokenClaims?.sub, })
...
app.post("/profile/authenticators/clear", async (req, res) => {
    try {
        if (req.oidc && req.oidc.user) {
            const response = await auth0ManagementClient.users.deleteAllAuthenticators(
                { id: req.oidc.idTokenClaims?.sub }
            )
        }
    } catch (error) {
